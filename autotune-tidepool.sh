...
if [[ -z "$DIR" || -z "$TIDEPOOL_USERNAME" ]]; then
    echo "Usage: autotune-tidepool <--dir=myopenaps_directory> <--username=tidepool_username> [--start-days-ago=number_of_days] [--end-days-ago=number_of_days] [--start-date=YYYY-MM-DD] [--end-date=YYYY-MM-DD] [--xlsx=autotune.xlsx] [--log=(true)|false] [--categorize-uam-as-basal=true|(false)]"
exit 1
fi
...
if [[ -z "$UNKNOWN_OPTION" ]] ; then # everything is ok
  echo "Running autotune-tidepool --dir=$DIR --ns-host=$NIGHTSCOUT_HOST --start-date=$START_DATE --end-date=$END_DATE --categorize-uam-as-basal=$CATEGORIZE_UAM_AS_BASAL"
else
  echo "Unknown options. Exiting"
  exit 1
fi

# Get the data from tidepool and extract the pump data
node tidepool-cli.js download $TIDEPOOL_USERNAME tidepool_download_temp.json
node tidepool-cli.js extract_openaps_pump_settings tidepool_download_temp.json openaps/settings/pumpprofile.json $START_DATE $END_DATE
cp openaps/settings/pumpprofile.json openaps/settings/autotune.json || die "Cannot copy settings/pumpprofile.json"
...
do
    ...
    node ../../tidepool-cli.js extract_nightscout_entries ../../tidepool_download_temp.json ns-entries.$i.json $START_DATE $END_DATE
    ...
    node ../../tidepool-cli.js extract_nightscout_treatments ../../tidepool_download_temp.json ns-treatments.$i.json $START_DATE $END_DATE
